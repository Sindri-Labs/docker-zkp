...
snarkjs r1cs info circom-files/circuit.r1cs

# Print binary file metadata
snarkjs file info circom-files/circuit.r1cs 

# List all of the constraints in the circuit
snarkjs r1cs print circom-files/circuit.r1cs circom-files/circuit.sym

# Transpile R1CS format into a human-readable JSON file
snarkjs r1cs export json circom-files/circuit.r1cs /tmp/snarkjs/r1cs/output.json

# Witness Utilities
mkdir /tmp/snarkjs/wtns

# Use Snarkjs CLI witness generator
snarkjs wtns calculate circom-files/circuit.wasm  circom-files/input.json /tmp/snarkjs/wtns/witness.wtns

# Use Snarkjs CLI witness generator in debug mode
snarkjs wtns debug circom-files/circuit.wasm  circom-files/input.json /tmp/snarkjs/wtns/witness.wtns circom-files/circuit.sym
...
snarkjs wtns check circom-files/circuit.r1cs circom-files/witness.wtns
...
for curve in BN254 BLS12-381; do
    ...
    then
        ...
        snarkjs groth16 setup circom-files/circuit.r1cs /tmp/snarkjs/BN254/final.ptau /tmp/snarkjs/BN254/groth16/circuit_1.zkey
        ...
        snarkjs zkey verify circom-files/circuit.r1cs /tmp/snarkjs/BN254/final.ptau /tmp/snarkjs/BN254/groth16/circuit_3.zkey
        ...
        snarkjs groth16 fullprove circom-files/input.json circom-files/circuit.wasm /tmp/snarkjs/BN254/groth16/final.zkey /tmp/snarkjs/BN254/groth16/proofA.json /tmp/snarkjs/BN254/groth16/publicA.json
        ...
        snarkjs groth16 prove /tmp/snarkjs/BN254/groth16/final.zkey circom-files/witness.wtns /tmp/snarkjs/BN254/groth16/proofB.json /tmp/snarkjs/BN254/groth16/publicB.json
        ...
        for backend in plonk fflonk; do
                ...
                snarkjs $backend setup circom-files/circuit.r1cs /tmp/snarkjs/$curve/final.ptau /tmp/snarkjs/$curve/$backend/circuit.zkey
                ...
                # End-to-end proof (no preconstructed witness)
                snarkjs $backend fullprove circom-files/input.json circom-files/circuit.wasm /tmp/snarkjs/$curve/$backend/circuit.zkey /tmp/snarkjs/$curve/$backend/proofA.json /tmp/snarkjs/$curve/$backend/publicA.json
                ...
                # Create a proof from a serialized witness
                snarkjs $backend prove /tmp/snarkjs/$curve/$backend/circuit.zkey circom-files/witness.wtns /tmp/snarkjs/$curve/$backend/proofB.json /tmp/snarkjs/$curve/$backend/publicB.json
